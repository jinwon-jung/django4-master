...
'''모델클래스에 없는 입력양식 생성
forms.py에 들어있는 XXXXField 객체를 클래스 변수에 저장헤서 구현
모델클래스에서 사용했던 클래스이름과 유사하나, 내부 기능이 다르기 때문에 주의해야함
models.XXXXField : 데이터베이스에 어떤 데이터를 저장할 지 지정하는 클래스
forms.XXXXField : <input>태그를 만들 때 어떤 형식으로 제공할지 지정하는 클래스

forms.XXXInput : <input>태그에 입력 형식을 바꿀 수 있는 클래스로 widget 매개변수(모델클래스와 연동한 경우 widgets)에 지정할 수 있음
forms.PasswordInput : 택스트 형태의 입력을 패스워드  형식으로 바꿔주는 위젯

'''
class SignupForm(forms.models.ModelForm):
    #label : HTML코드로 변환할 때, <label>태그 안에 들어갈 설명을 지정할 수 있는 변수
    #widget : HTML코드로 변환할 때, type속성값을 위젯으로 변환할 때 사용하는 변수
