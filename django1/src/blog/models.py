...
class Post(models.Model):
    ...
    u = models.ForeignKey(User, on_delete=models.PROTECT)
    #글자수 제한이 없는 저장공간을 설정하는 클래스
    #null : 데이터베이스에 해당변수값이 없어도 저장되도록 설정하는 매개변수
    #blank : 사용자가 입력양식에 해당변수값을 빈칸으로 둬도 폼을 서버에 전달할 
    #수 있도록 설정하는 매개변수, null과 blank 매개변수는 모든 Field클래스에서 사용가능
...
class PostImage(models.Model):
    ...
    #단, 해당 필드를 사용할려면 Pillow 모듈이 파이썬에 설치가 되어있어야함
