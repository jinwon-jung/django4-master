...
class Choice(models.Model):
    ...
    #Choice 객체가 연결한 Question객체가 삭제가되면, 자신도 삭제되도록 설정
...
-> Choice객체.q.name 이나 q.pub_date로 연결한 Question객체의 변수값을 사용할수잇음

매개변수 on_delete :연결된 모델클래스의 객체가 삭제될때 연결한 객체들을 어떻게 처리할지
지정하는 변수.
models.CASCADE : 연결된 모델클래스의 객체가 삭제될때,같이 삭제
models.PROTECT : 연결된 모델클래스가 삭제되지않도록 막아줌
models.SET_NULL : 연결된 객체가 삭제되면 아무것도 연결하지않은 상태 유지
